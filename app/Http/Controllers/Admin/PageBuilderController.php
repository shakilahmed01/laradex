...
class PageBuilderController extends Controller
{
    ...
    {
        $pdata = Page::where('tempname','templates.basic.')->get();
    ...
    public function managePagesSave(Request $request){
        ...
        $exist = Page::where('tempname', 'templates.basic.')->where('slug', slug($request->slug))->exists();
        if($exist){
            $notify[] = ['error', 'This page already exists on your current template. Please change the slug.'];
            return back()->withNotify($notify);
        }
        $page = new Page();
        $page->tempname = 'templates.basic.';
